...
namespace MWMechanics
{
        ...
        for (auto spellIt = mSpells.begin(); spellIt != mSpells.end();)
        {
            ...
            if (spellIt->mEffects.empty())
            {
                ...
                */
                ...
                {
                    mwmp::Main::get().getLocalPlayer()->sendSpellsActiveRemoval(spellIt->mId,
                        MechanicsHelper::isStackingSpell(spellIt->mId), spellIt->mTimeStamp);
                }
                else
                {
                    ...
                    if (mwmp::Main::get().getCellController()->isLocalActor(actorPtr))
                        mwmp::Main::get().getCellController()->getLocalActor(actorPtr)->sendSpellsActiveRemoval(spellIt->mId,
                            MechanicsHelper::isStackingSpell(spellIt->mId), spellIt->mTimeStamp);
                }
                /*
                    End of tes3mp addition
    ...
    void ActiveSpells::addTes3mpSpell(const ActiveSpellParams& params, const MWWorld::TimeStamp timestamp, bool sendPacket, bool stack)
    {
        ActiveSpellParams tmp = params;   // make a mutable copy
        tmp.mTimeStamp = timestamp;

        if (!stack)
        {
            // Remove any existing spell with this ID *before* inserting the new one
            for (auto spellIt = mSpells.begin(); spellIt != mSpells.end();)
            if (spellIt != mSpells.end())
                mSpells.erase(spellIt);

            // Also search in mQueue because spells may not be processed yet
            for (auto q = mQueue.begin(); q != mQueue.end();)
            {
                if (q->mId == params.mId)
                    q = mQueue.erase(q);
                else
       ...
       */
        if (sendPacket)
        {
            if (this == &MWMechanics::getPlayer().getClass().getCreatureStats(MWMechanics::getPlayer()).getActiveSpells())
            {
                mwmp::Main::get().getLocalPlayer()->sendSpellsActiveAddition(tmp.getId(), merge, tmp);
            }
            else
            {
                MWWorld::Ptr actorPtr = MWBase::Environment::get().getWorld()->searchPtrViaActorId(getActorId());
                ...
                if (mwmp::Main::get().getCellController()->isLocalActor(actorPtr))
                    mwmp::Main::get().getCellController()->getLocalActor(actorPtr)->sendSpellsActiveAddition(tmp.getId(), stack, tmp);
            }
        }
        /*
            End of tes3mp addition
        */
        addSpell(tmp);                    // queue it
    }
    ...
    */
	...
	void ActiveSpells::purgeEffectByArg(const MWWorld::Ptr& ptr, short effectId, int effectArg)
    {
        bool removedSpell = false;
        for(auto spellIt = mSpells.begin(); spellIt != mSpells.end();)
        {
			for(auto it = spellIt->mEffects.begin(); it != spellIt->mEffects.end();)
            {
                if (it->mEffectId == effectId && it->mArg == effectArg)
					removedSpell = applyPurges(ptr, &spellIt, &it);
					if(removedSpell)
						break;
                else
    ...
    */
    float ActiveSpells::getEffectDuration(short effectId, std::string sourceId)
    {
        for (auto spellIt = mSpells.begin(); spellIt != mSpells.end();)
        {
            if (sourceId.compare(spellIt->mId) == 0)
                for (auto it = spellIt->mEffects.begin(); it != spellIt->mEffects.end();)
                {
